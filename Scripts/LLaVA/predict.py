...
class Predictor(BasePredictor):
    ...
    def predict(
        self,
        image: Path = Input(description="file_path"),
        prompt: str = Input(description="<prompt_template>"),
        top_p: float = Input(description="top-p", ge=0.0, le=1.0, default=1.0),
        temperature: float = Input(description="temperature", default=0.2, ge=0.0),
        max_tokens: int = Input(description="token", default=1024, ge=0),
    ) -> ConcatenateIterator[str]:
        """Run a single prediction on the model"""

        conv_mode = "llava_version"
